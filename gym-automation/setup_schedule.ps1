...
# 오픈 작업: 매일 05:00 유튜브 재생
$openAction  = New-ScheduledTaskAction -Execute "$scriptDir\gym_open.bat"
$openTrigger = New-ScheduledTaskTrigger -Daily -At "05:00AM"
...
Register-ScheduledTask `
    ...
    -RunLevel Highest `
    -Description "헬스장 오픈 - 유튜브 음악 자동 재생"

# 마감 작업: 매일 01:00 브라우저 종료 + 절전
...
Register-ScheduledTask `
    ...
    -RunLevel Highest `
    -Description "헬스장 마감 - 브라우저 종료 및 PC 절전"

Write-Host ""
Write-Host "✔ 등록 완료!" -ForegroundColor Green
Write-Host "  GymOpen  - 매일 오전 05:00 유튜브 재생"
Write-Host "  GymClose - 매일 오전 01:00 브라우저 종료 + 절전"
Write-Host ""
